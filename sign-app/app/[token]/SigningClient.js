...
// Signature font options — client picks one
const SIG_FONTS = [
...
export default function SigningClient({ agreement }) {
    ...
    try {
      ...
      const res = await fetch('/api/sign', {
        ...
        body: JSON.stringify({
          ...
          caslConsent: casl,
          pdfBase64,
          sigFont1:    SIG_FONTS[sigFont1].font,
          sigFont2:    SIG_FONTS[sigFont2].font,
        }),
      })
  ...
  return (
    <div style={{ background: '#F7F5F0', minHeight: '100vh', padding: '2rem 1rem 4rem' }}>
      <div ref={agreementRef} style={{ maxWidth: 740, margin: '0 auto', background: 'white' }}>
        ...
        <div style={{ padding: '2.5rem 2.5rem' }}>
          ...
          <Section num={8} title="Electronic Communication & CASL Consent">
            ...
            {errors.casl && <div style={errStyle}>{errors.casl}</div>}
          </Section>
          ...
          <button
            ...
            style={{
              ...
              background: submitting ? '#8a9ab0' : NAVY,
...
function SignatureBlock({ label, name, email, phone, today, signed, sigFont, fontIndex, fonts, onSign, onUnsign, onFontChange, error }) {
  return (
    <div style={{ border: '1px solid #e8e3da', borderRadius: 12, padding: '1.25rem 1.5rem', marginBottom: 16, background: '#fafaf8' }}>
      <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, color: '#0F1F3D', marginBottom: 12, fontSize: 14 }}>
        {label}
      </div>
      ...
      {!signed ? (
        <div>
            ...
            {fonts.map((f, i) => (
              <button
                ...
                style={{
                  ...
                  borderColor: fontIndex === i ? '#0F1F3D' : '#e8e3da',
                  background: fontIndex === i ? '#0F1F3D' : 'white',
                  color: fontIndex === i ? 'white' : '#5a6a7a',
                  fontSize: 12, cursor: 'pointer',
                  fontFamily: f.font, fontStyle: f.style || 'normal',
                }}
              >
                {name}
              </button>
          ...
          {/* Preview + click to adopt */}
          <div
            onClick={onSign}
            style={{
              border: '1px dashed #b0b8c4', borderRadius: 8, padding: '18px 16px',
              cursor: 'pointer', background: 'white', textAlign: 'center',
              transition: 'border-color 0.15s',
            }}
          >
            <div style={{ fontFamily: sigFont.font, fontStyle: sigFont.style || 'normal', fontSize: 32, color: '#0F1F3D', marginBottom: 6 }}>
              {name}
            </div>
            <div style={{ fontSize: 11, color: '#8a9ab0', fontFamily: 'DM Mono, monospace' }}>
              Click to adopt this as your signature
            </div>
          </div>
        ...
        <div>
          <div style={{ border: '1px solid #e8e3da', borderRadius: 8, padding: '16px', background: 'white', position: 'relative' }}>
            <div style={{ fontSize: 10, color: '#8a9ab0', fontFamily: 'DM Mono, monospace', marginBottom: 4, textTransform: 'uppercase', letterSpacing: 1 }}>
              Signed by
            </div>
            <div style={{ fontFamily: sigFont.font, fontStyle: sigFont.style || 'normal', fontSize: 30, color: '#0F1F3D' }}>
              {name}
            </div>
            ...
            <button
              onClick={onUnsign}
              style={{ position: 'absolute', top: 10, right: 12, background: 'none', border: 'none', fontSize: 11, color: '#8a9ab0', cursor: 'pointer', fontFamily: 'DM Mono, monospace' }}
            >
              Change
            </button>
          </div>
...
const ulStyle = { paddingLeft: 18, lineHeight: 2.1, marginTop: 8 }
